...
func NewFieldIntMedata(modelFieldID, id string) *FieldIntMetadata {
	return &FieldIntMetadata{FieldMetadata: FieldMetadata{modelId: modelFieldID, id: id, dataType: FieldTypeInt}}
}
...
func castInt(fieldID string, field reflect.Value) (int64, error) {
	...
	case reflect.Int64:
		...
		if !ok {
			return 0, fmt.Errorf(ErrValCast, fieldID, "int64")
		}
	case reflect.Int32:
		val32, ok := field.Interface().(int32)
		if !ok {
			return 0, fmt.Errorf(ErrValCast, fieldID, "int32")
		}
		val = int64(val32)
	case reflect.Int16:
		val16, ok := field.Interface().(int16)
		if !ok {
			return 0, fmt.Errorf(ErrValCast, fieldID, "int16")
		}
		val = int64(val16)
	case reflect.Int8:
		val8, ok := field.Interface().(int8)
		if !ok {
			return 0, fmt.Errorf(ErrValCast, fieldID, "int8")
		}
		val = int64(val8)
	case reflect.Int:
		val0, ok := field.Interface().(int)
		if !ok {
			return 0, fmt.Errorf(ErrValCast, fieldID, "int")
		}
		val = int64(val0)
	case reflect.Float64:
		valFl, ok := field.Interface().(float64)
		if !ok {
			return 0, fmt.Errorf(ErrValCast, fieldID, "float64")
		}
		val = int64(valFl)
	default:
		return 0, fmt.Errorf(ErrValCast, fieldID, "int64")
...
func (f FieldIntMetadata) Validate(field reflect.Value) (bool, error) {
	...
	if ok {
		...
		elem := field.Elem()
		if !elem.IsValid() {
			return true, fmt.Errorf(ErrValCast, f.ModelID(), "int64")
		}
...
func (f FieldIntMetadata) CheckValue(field reflect.Value, val int64) error {
	if f.MinValue() != nil && val < *f.MinValue() {
		return fmt.Errorf(ErrValTooSmall, f.Descr())
	}
	if f.MaxValue() != nil && val > *f.MaxValue() {
		return fmt.Errorf(ErrValTooBig, f.Descr())
	}
