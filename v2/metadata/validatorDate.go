...
func (f FieldDateMetadata) Validate(field reflect.Value) (bool, error) {
	...
	if field.Kind() == reflect.Ptr && field.IsNil() {
		...
		field = field.Elem()
		if !field.IsValid() {
			return true, fmt.Errorf(ErrValCast, f.ModelID(), "date")
		}
	}

	txtField, ok := field.Interface().(string)
	if ok {
		if f.DataType() == FieldTypeDate && len(txtField) != 10 {
			return true, fmt.Errorf(ErrValCast, f.ModelID(), "string")

		}else if f.DataType() == FieldTypeTime && len(txtField) != 5 {
			return true, fmt.Errorf(ErrValCast, f.ModelID(), "string")

		}else if f.DataType() == FieldTypeDatetime && len(txtField) != 19 {
			return true, fmt.Errorf(ErrValCast, f.ModelID(), "string")

		}else if f.DataType() == FieldTypeDatetimeTZ && len(txtField) < 20 {
			return true, fmt.Errorf(ErrValCast, f.ModelID(), "string")
		}
	...
	if !ok {
		return true, fmt.Errorf(ErrValCast, f.ModelID(), "time.Time")
	}
