...
func NewFieldTextMedata(modelFieldID, id string) *FieldTextMetadata {
	return &FieldTextMetadata{FieldMetadata: FieldMetadata{modelId: modelFieldID, id: id, dataType: FieldTypeText}}
}
...
func (f FieldTextMetadata) Validate(field reflect.Value) (bool, error) {
	...
	if ok {
		...
		if !ok {
			return true, fmt.Errorf(ErrValCast, f.ModelID(), "string")
		}
	}

	valLen := int64(len([]rune(val)))
	if f.MinLength() != nil && valLen < *f.MinLength() {
		return true, fmt.Errorf(ErrValLenTooShort, f.Descr())
	}

	if f.MaxLength() != nil && valLen > *f.MaxLength() {
		return true, fmt.Errorf(ErrValLenTooLong, f.Descr())
	}

	if f.FixLength() != nil && valLen != *f.FixLength() {
		return true, fmt.Errorf(ErrValLenNotFix, f.Descr())
	}
	...
	if f.RegExp() != "" {
		...
		if !match {
			return true, fmt.Errorf(ErrValRegExp, f.Descr())
		}
	}
	...
	if len(f.valList) > 0 {
		...
		if !res {
			return true, fmt.Errorf(ErrValValList, f.Descr())
		}
	}
