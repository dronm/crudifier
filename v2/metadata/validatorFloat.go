...
func NewFieldFloatMedata(modelFieldID, id string) *FieldFloatMetadata {
	return &FieldFloatMetadata{FieldMetadata: FieldMetadata{modelId: modelFieldID, id: id, dataType: FieldTypeFloat}}
}
...
func castFloat(fieldID string, field reflect.Value) (float64, error) {
	...
	case reflect.Float32:
		...
		if !ok {
			return 0, fmt.Errorf(ErrValCast, fieldID, "float32")
		}
		val = float64(val32)
	default:
		return 0, fmt.Errorf(ErrValCast, fieldID, "float64")
...
func (f FieldFloatMetadata) Validate(field reflect.Value) (bool, error) {
	...
	if ok {
		...
		elem := field.Elem()
		if !elem.IsValid() {
			return true, fmt.Errorf(ErrValCast, f.ModelID(), "float64")
		}
	...
	if f.MinValue() != nil && val < *f.MinValue() {
		return true, fmt.Errorf(ErrValTooSmall, f.Descr())
	}
	if f.MaxValue() != nil && val > *f.MaxValue() {
		return true, fmt.Errorf(ErrValTooBig, f.Descr())
	}
	prec := f.Precision()
	if prec > 0 {
		...
		if !(math.Abs(val-roundedValue) < 1e-9) {
			return true, fmt.Errorf(ErrValPrecision, f.Descr())
		}
	}
