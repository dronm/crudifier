...
func NewModelMetadata(model any) (*ModelMetadata, error) {
	...
	if modelType.Kind() != reflect.Struct {
		return nil, fmt.Errorf(ErrModelNotAPointerOrStruct, "NewModelMetadata")
	}
	...
	for i := 0; i < modelType.NumField(); i++ {
		...
		case FieldTypeDate:
			validator := NewFieldDateMedata(fieldID, fieldTagVal, FieldTypeDate)
			meta.Fields[fieldTagVal] = validator

		case FieldTypeDatetime:
			validator := NewFieldDateMedata(fieldID, fieldTagVal, FieldTypeDatetime)
			meta.Fields[fieldTagVal] = validator

		case FieldTypeDatetimeTZ:
			validator := NewFieldDateMedata(fieldID, fieldTagVal, FieldTypeDatetimeTZ)
			meta.Fields[fieldTagVal] = validator
		default:
