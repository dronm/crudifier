		...
		},
		{
			...
			*pg.NewPgSelect(&TableC{},
			...
			CollectionParams{Count: 60,
				Sorter: []CollectionSorter{{"text_field", SortParAsc}, {"int_field", SortParDesc}},
				Filter: []CollectionFilter{{Join: FilterParJoinAnd,
					Fields: map[string]CollectionFilterField{"int_field": {FilterOperParEq, 1}},
				}}},
	...
	}{
		{
			...
			*pg.NewPgUpdate(&TableB{TextField: fields.NewFieldText("some text", true, false),
				...
				DateTimeField: fields.NewFieldDateTime(func() time.Time {
					t, _ := time.Parse(fields.FormatDateTime, "2024-12-25T07:30:00")
					return t
				}(), true, false),
				DateTimeTZField: fields.NewFieldDateTimeTZ(func() time.Time {
					t, _ := time.Parse(fields.FormatDateTimeTZ1, "2024-12-25T07:30:00.000-05")
					return t
				}(), true, false),
			}),
			"",
...
type TableSrvOnly struct {
	ID fields.FieldInt `json:"id" primaryKey:"true" srvCalc:"true"`
}

func (t TableSrvOnly) Relation() string {
	return "table_srv_only"
}

func (t TableSrvOnly) CollectionAgg() any {
	return nil
}

func TestPrepareInsertNoFields(t *testing.T) {
	insert := pg.NewPgInsert(&TableSrvOnly{})
	if err := PrepareInsertModel(insert); err == nil {
		t.Fatal("expected error for insert with no insertable fields")
	}
}

func TestPrepareUpdateNoFields(t *testing.T) {
	update := pg.NewPgUpdate(&TableB{})
	keyModel := &struct {
		ID fields.FieldInt `json:"id" required:"true"`
	}{ID: fields.NewFieldInt(1, true, false)}

	if err := PrepareUpdateModel(keyModel, update); err == nil {
		t.Fatal("expected error for update with no assignable fields")
	}
}

func TestPrepareFetchCollectionRejectsUnsafeFieldExpr(t *testing.T) {
	selectModel := pg.NewPgSelect(&TableC{}, &pg.PgFilters{}, &pg.PgSorters{}, &pg.PgLimit{})
	params := CollectionParams{
		Sorter: []CollectionSorter{{
			Field:  "text_field->>'x') DESC NULLS LAST",
			Direct: SortParAsc,
		}},
	}

	if err := PrepareFetchModelCollection(selectModel, params); err == nil {
		t.Fatal("expected invalid field expression error")
	}
}
