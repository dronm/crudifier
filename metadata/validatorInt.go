...
func (f FieldIntMetadata) Validate(field reflect.Value) (bool, error) {
	...
	if ok {
		...
		case reflect.Int64:
			...
			if !ok {
				return true, &IntervalServerError{ErrText: fmt.Sprintf(ER_VAL_CAST, f.ModelID(), "int64")}
			}
		case reflect.Int32:
			val32, ok := field.Interface().(int32)
			if !ok {
				return true, &IntervalServerError{ErrText: fmt.Sprintf(ER_VAL_CAST, f.ModelID(), "int32")}
			}
			val = int64(val32)
		case reflect.Int16:
			val16, ok := field.Interface().(int16)
			if !ok {
				return true, &IntervalServerError{ErrText: fmt.Sprintf(ER_VAL_CAST, f.ModelID(), "int16")}
			}
			val = int64(val16)
		case reflect.Int8:
			val8, ok := field.Interface().(int8)
			if !ok {
				return true, &IntervalServerError{ErrText: fmt.Sprintf(ER_VAL_CAST, f.ModelID(), "int8")}
			}
			val = int64(val8)
		case reflect.Int:
			val0, ok := field.Interface().(int)
			if !ok {
				return true, &IntervalServerError{ErrText: fmt.Sprintf(ER_VAL_CAST, f.ModelID(), "int")}
			}
			val = int64(val0)
		default:
			return true, &IntervalServerError{ErrText: fmt.Sprintf(ER_VAL_CAST, f.ModelID(), "int64")}
